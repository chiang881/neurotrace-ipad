...
public enum ParkinsonFeatureError: Error, Equatable {
    case insufficientSamples(testID: Int)
    case nonFiniteFeature(name: String)
}

public enum ParkinsonFeatureExtractor {
    ...
    public static func extract(
        from samples: [HandwritingSample]
        ...
        for (name, value) in zip(featureNames, values) {
            guard value.isFinite else {
                throw ParkinsonFeatureError.nonFiniteFeature(name: name)
            }
            result[name] = value
        }
        return result
    ...
    private static func extractTest(
        _ samples: [HandwritingSample],
        testID: Int
    ) throws -> [Double] {
        let selected = samples.filter { $0.testID == testID }
        guard selected.count >= 3 else {
            throw ParkinsonFeatureError.insufficientSamples(testID: testID)
        }

        var monotonic: [HandwritingSample] = []
