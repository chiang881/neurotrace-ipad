...
public actor FakeSubjectRepository: SubjectRepository {
    private var values: [ParchmentDomain.Subject]
    private var continuations: [UUID: AsyncStream<[ParchmentDomain.Subject]>.Continuation] = [:]

    public init(values: [ParchmentDomain.Subject] = []) { self.values = values }

    public func subjects(matching query: String) -> [ParchmentDomain.Subject] {
        query.isEmpty ? values : values.filter { $0.code.localizedCaseInsensitiveContains(query) }
    }

    public func subject(id: UUID) -> ParchmentDomain.Subject? { values.first { $0.id == id } }

    public func save(_ subject: ParchmentDomain.Subject) {
    ...
    public func updates() -> AsyncStream<[ParchmentDomain.Subject]> {
        let id = UUID()
...
public actor FakeSessionRepository: SessionRepository {
    private var values: [ParchmentDomain.Session]
    public init(values: [ParchmentDomain.Session] = []) { self.values = values }

    public func sessions(status: SessionStatus?) -> [ParchmentDomain.Session] {
        guard let status else { return values }
        return values.filter { $0.status == status }
    }

    public func session(id: UUID) -> ParchmentDomain.Session? { values.first { $0.id == id } }
    ...
    public func resume(id: UUID) throws -> ParchmentDomain.Session {
