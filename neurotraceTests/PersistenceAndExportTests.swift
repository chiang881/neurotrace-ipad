...
final class PersistenceAndExportTests: XCTestCase {
    ...
    func testExportContainsRequiredFiles() async throws {
        ...
        let manifest = try JSONDecoder.neurotrace.decode(ExportManifest.self, from: manifestData)
        ...
        let sessionExport = try JSONDecoder.neurotrace.decode(SessionExport.self, from: sessionData)
    ...
    private func temporaryRoot() -> URL {
        let url = FileManager.default.temporaryDirectory
            .appending(path: "NeuroTraceTests/\(UUID().uuidString)", directoryHint: .isDirectory)
