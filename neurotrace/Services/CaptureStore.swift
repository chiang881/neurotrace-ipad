...
actor LocalCaptureStore: CaptureStore {
    ...
    init(fileManager: FileManager = .default, rootURL: URL? = nil) {
        ...
        if let rootURL {
            ...
            self.rootURL = NeuroTraceStorage.dataRoot(fileManager: fileManager)
        }
    ...
    func commit(
        ...
        if !samples.isEmpty {
            try JSONEncoder.neurotrace.encode(samples).write(
                to: samplesURL,
        ...
        if !taps.isEmpty {
            try JSONEncoder.neurotrace.encode(taps).write(
                to: tapsURL,
                options: [.atomic, .completeFileProtectionUntilFirstUserAuthentication]
            )
    ...
    func loadSamples(relativePath: String?) throws -> [PencilSample] {
        ...
        return try JSONDecoder.neurotrace.decode([PencilSample].self, from: Data(contentsOf: url))
    }

    func loadTaps(relativePath: String?) throws -> [TapEvent] {
        guard let url = absoluteURL(relativePath: relativePath) else { return [] }
        return try JSONDecoder.neurotrace.decode([TapEvent].self, from: Data(contentsOf: url))
    }
