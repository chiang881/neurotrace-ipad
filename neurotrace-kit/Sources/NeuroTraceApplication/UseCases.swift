...
public struct Dashboard: Equatable, Sendable {
    ...
    public let activeSession: NeuroTraceDomain.Session?
    public let recentSubject: NeuroTraceDomain.Subject?

    public init(
        subjectCount: Int,
        completedSessionCount: Int,
        activeSession: NeuroTraceDomain.Session?,
        recentSubject: NeuroTraceDomain.Subject?
    ) {
...
public struct ManageSubjectUseCase: Sendable {
    ...
    public func list(query: String = "") async throws -> [NeuroTraceDomain.Subject] {
        try await repository.subjects(matching: query)
    }
    public func save(_ subject: NeuroTraceDomain.Subject) async throws { try await repository.save(subject) }
...
public struct ResumeSessionUseCase: Sendable {
    private let repository: any SessionRepository
    public init(repository: any SessionRepository) { self.repository = repository }
    public func callAsFunction(id: UUID) async throws -> NeuroTraceDomain.Session {
...
public struct ExportSessionUseCase: Sendable {
    ...
    public func callAsFunction(id: UUID) async throws -> URL {
        try await exporter.export(sessionID: id, schemaVersion: NeuroTraceDataContract.schemaVersion)
    }
}
